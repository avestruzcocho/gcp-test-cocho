CREATE OR REPLACE PROCEDURE `bnt-lakehouse-oro-des.ds_b_ant_oro.sp_bq_abandono_total_calc`(__BMONTH__ DATE)
...
create or replace table `bnt-lakehouse-oro-des.ds_paso.lgcf_at_all_pob` as 
WITH
  ONE AS (
          SELECT
            clie_fusio_id,
          ...
          FROM `bnt-lakehouse-plt-des.ds_b_cli_plt.t_b_cli_fusiones`  ------1mes
  ...
  FOUR AS (
            SELECT
              ...
              fecha_informacion,
              alta_fecha
            FROM `bnt-lakehouse-plt-des.ds_b_cli_plt.t_b_cli_datos_complementarios` A
            LEFT JOIN THREE B 
  ...
  AUX as (
          ...
          GROUP BY
            clie_final
          HAVING MIN(fecha_informacion) < DATE_ADD(__BMONTH__, INTERVAL -6 MONTH)
          AND    MAX(CASE WHEN fecha_informacion = LAST_DAY(__BMONTH__) THEN 1 ELSE 0 END) 
          =1 )
  SELECT
    clie_final,
    __BMONTH__ AS fecha_obs,
    min_fecha
  FROM AUX ;
---------------------------------------------------------------   PASO 2   -------------------------------------------------------

create or replace table `bnt-lakehouse-oro-des.ds_paso.lgcf_at_nomina` as 
WITH
  ONE AS (
          SELECT
            DISTINCT clie_fusio_id,
            clie_perm_numero
          FROM `bnt-lakehouse-plt-des.ds_b_cli_plt.t_b_cli_fusiones`)
          ,
  TWO AS (
          SELECT
            DISTINCT clie_final,
            clie_fusio_id
          FROM `bnt-lakehouse-oro-des.ds_paso.lgcf_at_all_pob`
  ...
  THREE AS (
            ...
            FROM `bnt-lakehouse-plt-des.ds_b_cap_plt.t_b_cap_dispersion_nomina_m`
            WHERE fecha_informacion BETWEEN DATE_ADD(__BMONTH__, INTERVAL -9 MONTH)
              AND LAST_DAY(__BMONTH__)
            GROUP BY cliente_id)
           ,
  FOUR AS (
            SELECT
...
create or replace table `bnt-lakehouse-oro-des.ds_paso.lgcf_at_all_productos` as 
WITH
  ONE AS (
          SELECT
            DISTINCT clie_fusio_id,
  ...
  THREE AS (
            ...
            FROM `bnt-lakehouse-plt-des.ds_b_cli_plt.t_b_cli_productos`
            WHERE estatus_tipo NOT IN ('VENCIDO', 'LIQUIDADO', 'CANCELADA') 
            AND fecha_informacion BETWEEN DATE_ADD(__BMONTH__, INTERVAL -5 MONTH) AND LAST_DAY(__BMONTH__)
...
WHERE fecha_informacion = LAST_DAY(__BMONTH__);

---------------------------------------------------------------   PASO 4  -------------------------------------------------------

create or replace table `bnt-lakehouse-oro-des.ds_paso.lgcf_at_vista_datos` as 
WITH
  zero AS (
          SELECT
            cliente_dwh_id,
          ...
          FROM `bnt-lakehouse-plt-des.ds_b_dwh_cap_plt.t_b_dwh_cap_saldo_vista`  
          WHERE pdimtiempo_fecha BETWEEN DATE_ADD(__BMONTH__, INTERVAL -9 MONTH) AND LAST_DAY(__BMONTH__))
SELECT
...
create or replace table `bnt-lakehouse-oro-des.ds_paso.lgcf_at_vista` as 
WITH
  ONE AS(
          SELECT
            DISTINCT clie_fusio_id,
  ...
  THREE AS (
          ...
          FROM `bnt-lakehouse-oro-des.ds_paso.lgcf_at_vista_datos`
          WHERE fecha BETWEEN DATE_ADD(__BMONTH__, INTERVAL -9 MONTH) AND LAST_DAY(__BMONTH__)
  ...
  FOUR AS (
          SELECT
            ...
            fechaultmov
          FROM THREE
          LEFT JOIN `bnt-lakehouse-plt-des.ds_b_dwh_cli_plt.t_b_dwh_cli_datos_basicos` USING (pdimcliente_id))
          ,
  five AS (
          SELECT
            num_clie,
...
---------------------------------------------------------------   PASO 6  -------------------------------------------------------
create or replace table `bnt-lakehouse-oro-des.ds_paso.lgcf_at_creditos` as 
WITH
  ONE AS (
          SELECT
            DISTINCT clie_fusio_id,
  ...
  THREE AS (
          ...
          AND fecha_informacion BETWEEN DATE_ADD(__BMONTH__, INTERVAL -9 MONTH) AND LAST_DAY(__BMONTH__) 
          GROUP BY cliente_id)
          ,
  FOUR AS (
          SELECT
...
create or replace table `bnt-lakehouse-oro-des.ds_paso.lgcf_at_all_tdc` as 
WITH
  ONE AS (
          SELECT
            ambs_acct_id,
            SUM(sdo_total_fin_monto) S
          FROM `bnt-lakehouse-plt-des.ds_b_col_plt.t_b_col_tdc_mensual`
          WHERE mes_fecha BETWEEN DATE_ADD(__BMONTH__, INTERVAL -23 MONTH) AND LAST_DAY(__BMONTH__)
  ...
  TWO AS (
          ...
          WHERE mes_fecha = DATE_ADD(__BMONTH__, INTERVAL -5 MONTH)),
  THREE AS (
          SELECT
          ambs_acct_id
          FROM `bnt-lakehouse-plt-des.ds_b_col_plt.t_b_col_tdc_mensual` A
          INNER JOIN ONE B USING (ambs_acct_id)
          LEFT JOIN TWO C USING (ambs_acct_id)
          WHERE mes_fecha = __BMONTH__ AND (CLASIFICACION1_ IS NULL OR CLASIFICACION1_ = 'SI')),
  FOUR AS (
          SELECT
            ambs_acct_id,
            mes_fecha,
          ...
          INNER JOIN THREE
          USING (ambs_acct_id)
          WHERE mes_fecha BETWEEN DATE_ADD(__BMONTH__, INTERVAL -5 MONTH) AND __BMONTH__),
  FIVE AS (
          SELECT
            ambs_acct_id,
            mes_fecha,
  ...
  six AS (
          ...
          FROM five
          WHERE mes_fecha = __BMONTH__),
  SEVEN AS (
          SELECT
            cliente_id,
            a.*
          FROM six a
          LEFT JOIN `bnt-lakehouse-plt-des.ds_b_cli_plt.t_b_cli_productos` b 
            ON CAST(ambs_acct_id AS NUMERIC) = cuenta_id AND fecha_informacion = LAST_DAY(__BMONTH__)),
  EIGHT AS (
          SELECT
            DISTINCT clie_fusio_id,
            clie_perm_numero
...
create or replace table `bnt-lakehouse-oro-des.ds_paso.lgcf_at_all_tabla`  --LGCF_AT_all_TABLA_8
(
clie_final numeric,
fecha_obs date,
min_fecha date,
...
INSERT INTO `bnt-lakehouse-oro-des.ds_paso.lgcf_at_all_tabla` (
SELECT * FROM `bnt-lakehouse-oro-des.ds_paso.lgcf_at_all_pob`
LEFT JOIN `bnt-lakehouse-oro-des.ds_paso.lgcf_at_nomina` USING (CLIE_FINAL)
LEFT JOIN `bnt-lakehouse-oro-des.ds_paso.lgcf_at_all_productos` USING (CLIE_FINAL)
LEFT JOIN `bnt-lakehouse-oro-des.ds_paso.lgcf_at_vista` USING (CLIE_FINAL)
LEFT JOIN `bnt-lakehouse-oro-des.ds_paso.lgcf_at_creditos` USING (CLIE_FINAL)
LEFT JOIN `bnt-lakehouse-oro-des.ds_paso.lgcf_at_all_tdc` USING (CLIE_FINAL));


create or replace table `bnt-lakehouse-oro-des.ds_paso.lgcf_at_all_month` as 
--TRUNCATE TABLE `ds_paso.lgcf_at_all_month`;
--INSERT INTO `ds_paso.lgcf_at_all_month`
WITH
  zero AS (
          SELECT
            cliente_id,
            MIN(nac_fecha) fecha_nac
          FROM `bnt-lakehouse-plt-des.ds_b_cli_plt.t_b_cli_datos_complementarios`
          GROUP BY cliente_id)
SELECT
  clie_final,
  fecha_obs,
  ...
  FROM `bnt-lakehouse-oro-des.ds_paso.lgcf_at_all_tabla`
  ...
  delete from `bnt-lakehouse-oro-des.ds_b_ant_oro.t_b_ant_ins_abn_tot_hist`
  where PARSE_DATE('%Y%m%d', CAST(fecha_informacion AS STRING)) = __BMONTH__ ;
  
  ---Guardar tabla de entrada mensual en una tabla historica
  insert into `bnt-lakehouse-oro-des.ds_b_ant_oro.t_b_ant_ins_abn_tot_hist`   ---tabla historica
  ...
  WHERE fecha_informacion = __BMONTH__ ;
