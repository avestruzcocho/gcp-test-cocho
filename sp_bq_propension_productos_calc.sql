CREATE OR REPLACE PROCEDURE `bnt-lakehouse-oro-des.ds_b_ant_oro.sp_bq_propension_productos_calc`(fecha_parametro DATE)
BEGIN
...
------------------------------------------------------------------------------
CREATE OR REPLACE TABLE bnt-lakehouse-oro-des.ds_paso.t_paso1_ins_prop_prod_vxx AS
WITH
  a0 AS (
    SELECT *,
    ...
    FROM bnt-lakehouse-plt-des.ds_b_ant_plt.t_b_ant_modelin
      ) 
  , 
  a AS (
      SELECT
      cliente_id,-- AGREGADO PARA LOS LAGS
  ...
  one AS (
    ...
    FROM bnt-lakehouse-plt-des.ds_b_cli_plt.t_b_cli_segmentos a -- tabla con segmentos
    LEFT JOIN (
      SELECT DISTINCT *
      FROM bnt-lakehouse-plt-des.ds_b_dwh_cat_plt.t_b_dwh_cat_segmentos ) b -- tabla con etiquetas de segmentos
  ...
  SELECT
    ...
    LEFT JOIN bnt-lakehouse-plt-des.ds_b_cli_plt.t_b_cli_datos_complementarios e
    ON a.cliente_id = e.cliente_id AND a.fecha_obs= DATE_TRUNC(e.fecha_informacion,MONTH) 
    LEFT JOIN bnt-lakehouse-plt-des.ds_b_rie_plt.t_b_rie_ingreso_cliente f
    ON a.cliente_id = f.cliente_id AND a.fecha_obs= DATE_TRUNC(f.fecha_informacion, MONTH) 
    LEFT JOIN bnt-lakehouse-plt-des.ds_b_dwh_cli_plt.t_b_dwh_cli_datos_basicos j
    ON a.cliente_id = j.numerocif_id
    LEFT JOIN bnt-lakehouse-plt-des.ds_b_ant_plt.t_b_ant_codigos_postales k
...
CREATE OR REPLACE TABLE bnt-lakehouse-oro-des.ds_paso.t_paso2_ins_prop_prod_vxx AS
SELECT
  *,
...
from bnt-lakehouse-oro-des.ds_paso.t_paso1_ins_prop_prod_vxx;
  
CREATE OR REPLACE TABLE bnt-lakehouse-oro-des.ds_paso.t_paso3_ins_prop_prod_vxx AS
...
FROM bnt-lakehouse-oro-des.ds_paso.t_paso2_ins_prop_prod_vxx;

-- Se vacia la tabla que almacena el input del modelo de manera mensual
truncate table bnt-lakehouse-oro-des.ds_b_ant_oro.t_b_ant_ins_prop_prod_vxx;

-- se Inserta la información en la tabla mensual input del modelo
INSERT INTO `bnt-lakehouse-oro-des.ds_b_ant_oro.t_b_ant_ins_prop_prod_vxx` 
SELECT 
 CAST(cliente_id as INT64) AS cliente_id, 
...
FROM bnt-lakehouse-oro-des.ds_paso.t_paso3_ins_prop_prod_vxx;

  -- Depuracion de tablas temporales
  drop table bnt-lakehouse-oro-des.ds_paso.t_paso1_ins_prop_prod_vxx;
  drop table bnt-lakehouse-oro-des.ds_paso.t_paso2_ins_prop_prod_vxx;
  drop table bnt-lakehouse-oro-des.ds_paso.t_paso3_ins_prop_prod_vxx;

--EVITAR DUPLICADOS SALIDA INSUMO HISTORICO
 DELETE FROM bnt-lakehouse-oro-des.ds_b_ant_oro.t_b_ant_ins_prop_prod_hist
...
--EVITAR DUPLICADOS SALIDA PREDICION ML
 DELETE FROM bnt-lakehouse-oro-des.ds_b_ant_oro.t_b_ant_calif_prop_prod_hist_vxx
...
-- Guardar tabla de entrada mensual en una tabla historica
  INSERT INTO   `bnt-lakehouse-oro-des.ds_b_ant_oro.t_b_ant_ins_prop_prod_hist`
  SELECT * FROM `bnt-lakehouse-oro-des.ds_b_ant_oro.t_b_ant_ins_prop_prod_vxx`
   ;

--TRUNCATE 

TRUNCATE TABLE bnt-lakehouse-oro-des.ds_b_ant_oro.t_b_ant_calif_prop_prod_vxx;
TRUNCATE TABLE bnt-lakehouse-oro-des.ds_b_ant_oro.t_paso_pred_prop_prod_full;
